...
def DrawCanvas(canvas, ax, colors = "gray"):
    ...
    plt.text(P-0.4, P-0.2, r'$(P_E,P_E)$')
    plt.text(R+0.05, R, r'$(R_E,R_E)$')
    plt.text(T-0.1, S-0.1, r'$(T_E,S_E)$')
    plt.text(S-0.1, T+0.05, r'$(S_E,T_E)$')
...
if __name__ == "__main__":
    try:
        ...
        scale6 = tkinter.Scale(root, label='epsilon',orient='h', from_=0, to=500, command=partial(DrawCanvas, Canvas, ax1))
        scale6.grid(row=4, column=1, columnspan=1)
        scale7 = tkinter.Scale(root, label='xi', orient='h', from_=0, to=500, command=partial(DrawCanvas, Canvas, ax1))
